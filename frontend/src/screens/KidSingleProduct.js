...
import Womendetail from "./images/Womendetailcover.png";
import wpic from "./images/women.png"


import {
 
    KidMenReview,KidlistProductDetails
} from "../Redux/Actions/KidsAction";
import Loading from "../components/LoadingError/Loading";
import { NEW_REVIEW_SUCCESS } from "../Redux/Constants/KidsConstants";
import moment from "moment";
import MetaData from "./MetaData";
import Carousel from "react-material-ui-carousel";
import store from "../Redux/store";
import { addKidItemsToCart } from "../Redux/Actions/cartActions";
import KidProductBanner from "./images/KidProductBanner.png";
...
const KidSingleProduct = ({ match }) => {
  ...
  const KidproductDetail = useSelector((state) => state.KidproductDetail);
  const { loading, error, product } = KidproductDetail;
  const userLogin = useSelector((state) => state.userLogin);
  const { userInfo } = userLogin;
  const { success, error: reviewError } = useSelector(
    (state) => state.newWomenReview);
  const options = {
    size: "large",
  ...
  const reviewSubmitHandler = () => {
    ...
    dispatch(KidMenReview(myForm));

    setOpen(false);
  };
  const addToCartHandler = () => {
    dispatch(addKidItemsToCart(match.params.id, quantity));
  ...
  useEffect(() => {
    ...
    dispatch(KidlistProductDetails(productId));
  }, [dispatch, match.params.id, error, alert, reviewError, success]);

//   const addToCartHandler = () => {
//     dispatch(addWomenItemsToCart(match.params.id, quantity));
//     alert("Item Added To Cart");
//   };
  return (
    <>
...
<MobileMenu/>
      ...
      <div className="inner-banner1"></div>
                <Link to="/ChildrensProducts" >
                    <img src={KidProductBanner} />
                </Link>
                    ...
                    <div className="product-details-btn">
                      ...
                      <ul>
                        ...
                        <li className="icon wishlist-icon">
                          <a className="btn btn-transparent"><span></span></a>
                        </li>
...
export default KidSingleProduct;
